...
import { getSuperadminPartnerData } from "@/lib/supabase/queries/partners";
...
function SuperadminPartnersContent() {
  ...
  const fetchPartnerData = async () => {
    try {
      setLoading(true);
      const data = await getSuperadminPartnerData();
      setPartners(data.partners || []);
      setDeals(data.deals || []);
      setCommissions(data.commissions || []);
    } catch (err) {
      console.error("Failed to load partner data from PostgreSQL:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  const pendingApps = partners.filter(p => p.status === "pending_approval" || p.status === "pending");
  ...
  const totalCalculatedPayout = commissions.reduce((acc, c) => acc + (Number(c.commission_amount_inr) || 0), 0);

  const approvePartner = async (id: string) => {
    try {
      await db.from("partners").update({ status: "active", updated_at: new Date().toISOString() }).eq("id", id);
      setPartners(prev => prev.map(p => p.id === id ? { ...p, status: "active" } : p));
    } catch (e) {
      console.error("Failed to approve partner in DB:", e);
    }
  };

  const rejectPartner = async (id: string) => {
    try {
      await db.from("partners").update({ status: "rejected", updated_at: new Date().toISOString() }).eq("id", id);
      setPartners(prev => prev.map(p => p.id === id ? { ...p, status: "rejected" } : p));
    } catch (e) {
      console.error("Failed to reject partner in DB:", e);
    }
  };
    ...
    <div className="p-6 min-h-screen bg-[#F8FAFC] font-sans space-y-8">
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 border-b border-slate-200 pb-5">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 text-xs font-bold text-slate-700 bg-white border border-slate-300 hover:bg-slate-50 px-3.5 py-2 rounded-xl transition-all shadow-xs disabled:opacity-50"
          >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs flex items-center justify-between">
          <div>
            ...
            <div className="text-2xl font-black text-golden-700 mt-1">
              {totalCalculatedPayout > 0 ? `₹${(totalCalculatedPayout / 100000).toFixed(2)}L` : "₹0"}
            </div>
          </div>
          <div className="w-10 h-10 rounded-xl bg-golden-50 text-golden-700 border border-golden-200 flex items-center justify-center font-bold">
            💰
      ...
      {activeTab === "applications" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
            {loading ? (
              <div className="p-12 text-center text-xs text-slate-400">Loading partner applications from database...</div>
            ) : partners.length === 0 ? (
              <div className="p-12 text-center text-slate-500">
                ...
                <h3 className="font-bold text-slate-800 text-sm">No Partner Applications Yet</h3>
                <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                  When partners submit applications via <Link href="/partner" target="_blank" className="text-blue-600 underline">nisolai.com/partner</Link>, their submissions will appear here for review.
                </p>
              </div>
            ) : (
      ...
      {activeTab === "deals" && (
        <div className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
            {loading ? (
              <div className="p-12 text-center text-xs text-slate-400">Loading registered deals from database...</div>
            ) : deals.length === 0 ? (
              <div className="p-12 text-center text-slate-500">
                <div className="w-12 h-12 mx-auto mb-3 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400">
                  <ShieldCheck className="w-6 h-6" />
                </div>
                <h3 className="font-bold text-slate-800 text-sm">No Registered Deals Yet</h3>
                <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                  Registered partner prospect domains will appear here with timestamped conflict checks.
                </p>
              </div>
            ) : (
              <div className="overflow-x-auto">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {deals.map((d) => (
                      <tr key={d.id} className={d.status === "conflict_rejected" ? "bg-amber-50/40 hover:bg-amber-50" : "hover:bg-slate-50/50"}>
                        ...
                        <td className="p-4 font-medium text-slate-700">{d.partner_name || d.partner?.company_name || "Partner"}</td>
                        ...
                        <td className="p-4 text-slate-600 font-medium">
                          {d.protection_expires_at && d.protection_expires_at !== "N/A" ? (
                          ...
                          {d.status === "protected" ? (
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="text-[10px] text-amber-700">
                                Earlier Partner: <strong>{d.conflict_partner_name || "Existing Partner"}</strong>
                              </div>
                            </div>
                          )}
                        </td>
        ...
        <div className="space-y-4">
          ...
          {loading ? (
            <div className="p-12 text-center text-xs text-slate-400 bg-white rounded-2xl border border-slate-200">
              Loading active partner directory...
            </div>
            ...
            <div className="p-12 text-center text-slate-500 bg-white rounded-2xl border border-slate-200">
              ...
              <h3 className="font-bold text-slate-800 text-sm">No Active Partners Yet</h3>
              <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                Once partner applications are approved, active firms and their unique referral codes will appear here.
              </p>
            </div>
          ) : (
      ...
      {activeTab === "commissions" && (
        <div className="space-y-4">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs overflow-hidden">
            {loading ? (
              <div className="p-12 text-center text-xs text-slate-400">Loading commission payouts...</div>
            ) : commissions.length === 0 ? (
              <div className="p-12 text-center text-slate-500">
                <div className="w-12 h-12 mx-auto mb-3 rounded-2xl bg-slate-100 flex items-center justify-center text-slate-400">
                  <DollarSign className="w-6 h-6" />
                </div>
                <h3 className="font-bold text-slate-800 text-sm">No Commissions Recorded Yet</h3>
                <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                  Net-30 commissions will be automatically calculated when partner deals close.
                </p>
              </div>
            ) : (
              <table className="w-full text-xs text-left">
                ...
                <tbody className="divide-y divide-slate-100">
                  {commissions.map((c) => (
                    <tr key={c.id} className="hover:bg-slate-50/50">
                      <td className="p-4 font-bold text-[#0A1E3C]">{c.partner_name || c.partner?.company_name || "Partner"}</td>
                      <td className="p-4 text-slate-700">{c.target_company_name || c.deal?.target_company_name || "Client"}</td>
                      <td className="p-4 font-mono font-bold text-slate-800">₹{Number(c.deal_revenue_inr || 0).toLocaleString("en-IN")}</td>
                      <td className="p-4 font-bold text-golden-600">{c.commission_type}</td>
                      <td className="p-4 font-mono font-black text-emerald-700">₹{Number(c.commission_amount_inr || 0).toLocaleString("en-IN")}</td>
                      <td className="p-4 text-center">
                        <span className="px-2.5 py-1 rounded-full text-[10px] font-bold bg-golden-100 text-golden-800 border border-golden-300">
